...
```
Error: locator.hover: Unexpected token "" while parsing css selector "". Did you mean to CSS.escape it?
Call log:
  - waiting for
...
```ts
  1   | /**
  2   |  * BasePage — shared navigation helpers and common actions
  3   |  * ========================================================
  4   |  * Every page object extends this class so common utilities (goto, nav clicks,
  5   |  * waiting helpers) are available everywhere without duplication.
  6   |  */
  7   | 
  8   | import { Locator, Page, expect } from '@playwright/test';
  9   | 
  10  | export const BASE_URL = 'https://nagarjunreddykasu.github.io/web-automation-practice-site/';
  11  | 
  12  | export type PlaywrightRole = Parameters<Page['getByRole']>[0];
  13  | 
  14  | export type LocatorTarget =
  15  |   | string
  16  |   | Locator
  17  |   | { by: 'role'; role: PlaywrightRole; name?: string; exact?: boolean }
  18  |   | { by: 'label'; label: string; exact?: boolean }
  19  |   | { by: 'text'; text: string; exact?: boolean }
  20  |   | { by: 'placeholder'; placeholder: string; exact?: boolean }
  21  |   | { by: 'testId'; testId: string }
  22  |   | { by: 'altText'; altText: string; exact?: boolean };
  23  | 
  24  | export class BasePage {
  25  |   constructor(protected readonly page: Page) {}
  26  | 
  27  |   private isLocator(target: LocatorTarget): target is Locator {
  28  |     return typeof target !== 'string' && Object.prototype.hasOwnProperty.call(target, 'locator');
  29  |   }
  30  | 
  31  |   /**
  32  |    * Returns a Locator for a string selector, native locator, or helper-based locator.
  33  |    */
  34  |   protected getLocator(target: LocatorTarget): Locator {
  35  |     if (typeof target === 'string') {
  36  |       return this.page.locator(target);
  37  |     }
  38  | 
  39  |     if (this.isLocator(target)) {
  40  |       return target;
  41  |     }
  42  | 
  43  |     switch (target.by) {
  44  |       case 'role':
  45  |         return this.page.getByRole(target.role, { name: target.name, exact: target.exact });
  46  |       case 'label':
  47  |         return this.page.getByLabel(target.label, { exact: target.exact });
  48  |       case 'text':
  49  |         return this.page.getByText(target.text, { exact: target.exact });
  50  |       case 'placeholder':
  51  |         return this.page.getByPlaceholder(target.placeholder, { exact: target.exact });
  52  |       case 'testId':
  53  |         return this.page.getByTestId(target.testId);
  54  |       case 'altText':
  55  |         return this.page.getByAltText(target.altText, { exact: target.exact });
  56  |       default:
  57  |         return this.page.locator('');
  58  |     }
  59  |   }
  60  | 
  61  |   /** Navigate to the practice hub root */
  62  |   async goto(): Promise<void> {
  63  |     await this.page.goto(BASE_URL, { waitUntil: 'domcontentloaded' });
  64  |   }
  65  | 
  66  |   /** Navigate to a specific URL */
  67  |   async navigateToUrl(url: string): Promise<void> {
  68  |     await this.page.goto(url, { waitUntil: 'domcontentloaded' });
  69  |   }
  70  | 
  71  |   /** Click the target element */
  72  |   async click(target: string | Locator): Promise<void> {
  73  |     await this.getLocator(target).click();
  74  |   }
  75  | 
  76  |   /** Fill the target input element */
  77  |   async fill(target: string | Locator, value: string): Promise<void> {
  78  |     await this.getLocator(target).fill(value);
  79  |   }
  80  | 
  81  |   /// Hover over the target element
  82  |   async hover(target: string | Locator): Promise<void> {
> 83  |     await this.getLocator(target).hover();
      |                                   ^ Error: locator.hover: Unexpected token "" while parsing css selector "". Did you mean to CSS.escape it?
  84  |   }
  85  | 
  86  |   /** Get the text content of the target element */
  87  |   async getText(target: string | Locator): Promise<string> {
  88  |     return (await this.getLocator(target).textContent()) ?? '';
  89  |   }
  90  | 
  91  |   /** Get the input value of the target element */
  92  |   async getValue(target: string | Locator): Promise<string> {
  93  |     return await this.getLocator(target).inputValue();
  94  |   }
  95  | 
  96  |   /** Get attribute value from the target element */
  97  |   async getAttribute(target: string | Locator, name: string): Promise<string | null> {
  98  |     return await this.getLocator(target).getAttribute(name);
  99  |   }
  100 | 
  101 |   /** Determine if the target element is visible */
  102 |   async isVisible(target: string | Locator): Promise<boolean> {
  103 |     return await this.getLocator(target).isVisible();
  104 |   }
  105 | 
  106 |   /** Wait until the target element is visible */
  107 |   async waitForVisible(target: string | Locator, timeout?: number): Promise<void> {
  108 |     await this.getLocator(target).waitFor({ state: 'visible', timeout });
  109 |   }
  110 | 
  111 |   /** Wait until the target element is hidden */
  112 |   async waitForHidden(target: string | Locator, timeout?: number): Promise<void> {
  113 |     await this.getLocator(target).waitFor({ state: 'hidden', timeout });
  114 |   }
  115 | 
  116 |   /**
  117 |    * Click a navigation menu item by its `data-page` attribute value.
  118 |    * Waits for the corresponding page section to become visible.
  119 |    */
  120 |   async navigateTo(
  121 |     pageName:
  122 |       | 'welcome'
  123 |       | 'login'
  124 |       | 'register'
  ...
  131 |       | 'interactions',
  132 |   ): Promise<void> {
  133 |     const navLink = this.page.locator(`a[data-page="${pageName}"]`);
  134 |     await navLink.click();
  135 |     const section = this.page.locator(`#page-${pageName}`);
  136 |     await expect(section).toBeVisible();
  137 |   }
  138 | 
  139 |   /** Verify the active navigation link matches the expected page */
  140 |   async expectActiveNav(pageName: string): Promise<void> {
  141 |     const activeLink = this.page.locator('a.active[data-page]');
  142 |     await expect(activeLink).toHaveAttribute('data-page', pageName);
  143 |   }
  144 | 
  145 |   /** Return the page title text */
  146 |   async getPageTitle(): Promise<string> {
  147 |     return await this.page.title();
  148 |   }
  149 | }
```